...
const (
	...
	PlatformOllama      = domain.PlatformOllama
...
const (
	...
	SettingKeyRegistrationDomainLimitEnabled   = "registration_domain_limit_enabled"
	SettingKeyRegistrationDomainLimitPerDomain = "registration_domain_limit_per_domain"
	SettingKeyXSearchPricePerRequest           = "x_search_price_per_request"       // x_search 单次 USD 价格（decimal(20,10)）
	...
	// SettingKeyOpenAIRemoteCompactionV2Enabled controls the native Remote
	// Compaction V2 protocol. The legacy /responses/compact path remains
	// available when this switch is off. Missing values default to enabled.
	SettingKeyOpenAIRemoteCompactionV2Enabled = "openai_remote_compaction_v2_enabled"

	// SettingKeyGroupUsageRollupEnabled controls the group-usage rollup reader
	// and worker. When off, reads use the existing realtime path and the worker
	// pauses; rollup tables and invalidations remain intact for recovery.
	SettingKeyGroupUsageRollupEnabled = "group_usage_rollup_enabled"

	// SettingKeySalesPricingResolverEnabled independently controls the unified
	// sales-pricing resolver. When off, billing uses the legacy pricing path and
	// does not create new shadow or v2 pricing snapshots. Missing values default
	// to enabled for upgrade compatibility.
	SettingKeySalesPricingResolverEnabled = "sales_pricing_resolver_enabled"
	// SettingKeyOpenAITeamLinkedResolverEnabled controls Team-wide blocking and
	// cross-account recovery. Missing values default to enabled; a transient
	// read failure preserves the last confirmed state and fails closed initially.
	SettingKeyOpenAITeamLinkedResolverEnabled = "openai_team_linked_resolver_enabled"
